...
class Filtro(Resource):
    ...
    def get(self, card):
        self.card = card
        self.df_card = self._filter_by_card(self.card)
        self.df_groupedby = self._agrupar_marital_status()
    ...
    def _agrupar_marital_status(self):
        return self.df_card.groupby('Marital_Status').mean()[['Total_Trans_Amt']]
    ...
    def _generate_churn_education_plot(self):
        plt.figure(figsize=(10, 5))
        sns.countplot(x=self.df_card['Education_Level'], hue=self.df_card['Attrition_Flag'])
    ...
    def _generate_corr_plot(self):
        ...
        sns.heatmap(self.df_card.corr(), cmap='coolwarm', vmin=-1, vmax=1)
